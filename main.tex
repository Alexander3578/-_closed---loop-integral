...
\[ \oint_C \frac{e^z}{(z-i)^2(z+2)}dz =  2\pi i(\frac{e^z}{z+2})^\prime\bigg|_^z=i \]

\par\noindent что равно 
\[2\pi i(\frac{e^z(z+2)-e^z}{(z+2)^2})|{_^z=i} = \frac{2\pi i(i+1)}{(i+2)^2}e^i\]
\par\noindentб) в круг |z+2-i|<3 входят две точки i и -2. Расписываем интеграл как сумму двух инетгралов:  
\[\oint_C f(z)dz = \oint_{C_1} f(z)dz+\oint_{C_2} f(z)dz\]
\par\noindent где контуры {C$_1$} и {{С_2}} содержат по одной точке. Получаем:
